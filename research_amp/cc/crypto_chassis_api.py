...
def load_crypto_chassis_ohlcv_for_one_symbol(full_symbol: str) -> pd.DataFrame:
    ...
    # Build a request.
    r = safe_requests.get(f"https://api.cryptochassis.com/v1/ohlc/{cc_exchange_id}/{cc_currency_pair}?startTime=0"
    )
    # Get url with data.
...
def load_bid_ask_data_for_one_symbol(
    ...
    for date in list_of_dates:
        ...
        r = safe_requests.get(f"https://api.cryptochassis.com/v1/market-depth/{cc_exchange_id}/{cc_currency_pair}?startTime={date}"
        )
